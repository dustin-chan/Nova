...
function updateSliders() {
  ...
  speedRange.min = -levelDepth * 0.99;
  speedRange.max = levelDepth * 0.99;
  const speedMin = document.getElementById( 'speed-min' );
  speedMin.innerHTML = roundTo(speedRange.min, 100);
  const speedVal = document.getElementById( 'speed-value' );
  speedVal.innerHTML = roundTo(speed, 100);
  const speedMax = document.getElementById( 'speed-max' );
  speedMax.innerHTML = roundTo(speedRange.max, 100);
  sliders.push(
  ...
  sliders.forEach( domEl => {
    ...
    elValLi.innerHTML = roundTo( eval( domEl.id ), 100 );
...
function render() {
  ...
  if ( speed > levelDepth * 1.025 || speed < -levelDepth * 1.025 ) {
    if ( !cosmicInception ) easterEggBang();
  } else {
    choice = Math.random();
    if ( choice <= 0.777777 && choice > 0.777770 ) {
      if ( !cosmicInception ) easterEggBang();
    }
  }
