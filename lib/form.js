...
function createInputs( inputsAttrs ) {
  inputsAttrs.forEach( ( attrs ) => {
    ...
    const min = createElWithAttributes( 'li', rangeValsList, { 'class': 'range-min range-li' } );
    min.innerHTML = roundTo(attrs.min, 100);
    const value = createElWithAttributes( 'ul', rangeValsList, { 'class': 'range-value range-li' } );
    value.innerHTML = roundTo(attrs.value, 100);
    const max = createElWithAttributes( 'ul', rangeValsList, { 'class': 'range-max range-li' } );
